...
> = {
  ...
  featured: { label: "Spotlight", bg: "#D6E2FF", color: "#3D64EB", border: "#5178FF" },
  verified: { label: "Verified", bg: "#DCFCE7", color: "#16A34A", border: "#DCFCE7" },
  new_launch: { label: "New", bg: "#ECF2FF", color: "#3D64EB", border: "#5178FF" },
  editors_pick: { label: "Editor's Pick", bg: "#ECF2FF", color: "#3D64EB", border: "#D6E2FF" },
  trending: { label: "Rising", bg: "#FFFBEB", color: "#92400E", border: "#FBBF24" },
  pro_founder: { label: "Pro", bg: "#0C1830", color: "#F59E0B", border: "#475569" },
  community_pick: { label: "Community Pick", bg: "#FEF2F2", color: "#EF4444", border: "#FEE2E2" },
  best_value: { label: "Best Value", bg: "#DCFCE7", color: "#16A34A", border: "#DCFCE7" },
    ...
    <div className="flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((i) => (
        <Star
          ...
          fill={i <= fullStars ? "#F59E0B" : (i === fullStars + 1 && hasHalf ? "#FBBF24" : "#E2E8F0")}
          color={i <= fullStars ? "#F59E0B" : (i === fullStars + 1 && hasHalf ? "#FBBF24" : "#E2E8F0")}
        />
      ))}
...
function UpvoteButton({
    ...
    return (
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" fill="none" viewBox="0 0 16 16" style={{ flexShrink: 0 }}>
          <path fill={active ? "#FEF3C7" : "#FFFFFF"} stroke="currentColor" strokeWidth="1.5" d="M6.579 3.467c.71-1.067 2.132-1.067 2.842 0L12.975 8.8c.878 1.318.043 3.2-1.422 3.2H4.447c-1.464 0-2.3-1.882-1.422-3.2z" />
        </svg>
        {displayCount}
      </button>
    );
  ...
  return (
    ...
    >
      <svg xmlns="http://www.w3.org/2000/svg" width={isLarge ? 16 : 12} height={isLarge ? 16 : 12} fill="none" viewBox="0 0 16 16">
        <path fill={active ? "#FEF3C7" : "#FFFFFF"} stroke="currentColor" strokeWidth="1.5" d="M6.579 3.467c.71-1.067 2.132-1.067 2.842 0L12.975 8.8c.878 1.318.043 3.2-1.422 3.2H4.447c-1.464 0-2.3-1.882-1.422-3.2z" />
      </svg>
...
function PricingPill({ model }: { model: string }) {
  const styles: Record<string, { bg: string; color: string; border: string }> = {
    Free: { bg: "#DCFCE7", color: "#16A34A", border: "#DCFCE7" },
    Freemium: { bg: "#D6E2FF", color: "#3D64EB", border: "#5178FF" },
    "Open Source": { bg: "#ECF2FF", color: "#3D64EB", border: "#D6E2FF" },
    "Free Trial": { bg: "#ECF2FF", color: "#3D64EB", border: "#5178FF" },
  };
  ...
  return (
    <>
      ...
      <Link
        ...
        className="group flex items-center gap-3 px-3 py-3 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100/80 hover:border-slate-200 transition-all no-underline"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-0.5">
            <span className="text-[13px] font-bold text-slate-900 truncate">{tool.name}</span>
...
function StandardCard({ tool, rank, hideCategory = false }: ToolCardProps) {
  ...
  const safeBadges = Array.isArray(tool.badges) ? tool.badges : [];
  const visibleBadges = (safeBadges as BadgeType[]).filter(b => BADGE_CONFIG[b]).slice(0, 1);
  ...
  return (
    <>
      ...
      <Link
        href={`/tools/${tool.slug}`}
        className="group flex items-start gap-4 sm:gap-5 p-4 sm:p-5 rounded-2xl border bg-slate-50 transition-all no-underline relative overflow-hidden"
        style={{
          borderColor: "#E2E8F0",
          borderLeft: (tool.is_featured || safeBadges.includes('top_rated')) ? '3px solid #F59E0B' : undefined,
        }}
        onMouseEnter={(e) => {
          e.currentTarget.style.borderColor = "#CBD5E1";
          if (tool.is_featured || safeBadges.includes('top_rated')) {
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="text-[17px] sm:text-lg font-extrabold text-slate-900 tracking-tight truncate">
              {tool.name}
            </span>
        ...
        <div className="hidden sm:flex flex-col items-center justify-start gap-2 shrink-0 mt-0.5">
          ...
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              border: "1.5px solid",
              background: saved ? "#D6E2FF" : "#F8FAFC",
              borderColor: saved ? "#5178FF" : "#E2E8F0",
              color: saved ? "#3D64EB" : "#94A3B8",
              cursor: isPendingSave ? "wait" : "pointer",
              opacity: isPendingSave ? 0.6 : 1,
            }}
            onMouseEnter={(e) => {
              if (!saved) {
                e.currentTarget.style.borderColor = "#5178FF";
                e.currentTarget.style.background = "#ECF2FF";
              }
            }}
            onMouseLeave={(e) => {
              if (!saved) {
          ...
          >
            <Bookmark style={{ width: 14, height: 14 }} fill={saved ? "#3D64EB" : "none"} />
          </button>
          <button
            ...
            style={{
              ...
              borderRadius: 8,
              border: "1.5px solid",
              background: comparing ? "#ECF2FF" : "#F8FAFC",
              borderColor: comparing ? "#D6E2FF" : "#E2E8F0",
              color: comparing ? "#3D64EB" : "#94A3B8",
              cursor: "pointer",
            }}
            onMouseEnter={(e) => {
              if (!comparing) {
                e.currentTarget.style.borderColor = "#D6E2FF";
                e.currentTarget.style.background = "#ECF2FF";
              }
            }}
            onMouseLeave={(e) => {
        ...
        <div className="sm:hidden shrink-0 self-start mt-0.5 flex flex-col items-center gap-1.5">
          ...
          >
            <Bookmark style={{ width: 12, height: 12 }} fill={saved ? "#3D64EB" : "none"} />
          </button>
          <button
            onClick={handleCompare}
            className="flex items-center justify-center transition-all"
...
function FeaturedCard({ tool, rank }: ToolCardProps) {
  ...
  const safeBadges = Array.isArray(tool.badges) ? tool.badges : [];
  const visibleBadges = (safeBadges as BadgeType[]).filter(b => BADGE_CONFIG[b]).slice(0, 2);
  ...
  return (
    <>
      ...
      <Link
        href={`/tools/${tool.slug}`}
        className="group block rounded-2xl bg-slate-50 overflow-hidden transition-all no-underline h-full"
        style={{
      ...
      >
        ...
        <div className="p-4 sm:p-5 pb-3">
          ...
          <p
            className="text-sm text-slate-600 leading-[1.5] mt-2.5"
            style={{
              display: "-webkit-box",
              WebkitLineClamp: 2,
              WebkitBoxOrient: "vertical",
