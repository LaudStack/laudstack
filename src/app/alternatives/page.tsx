...
function SearchResultItem({
  ...
  return (
    ...
    >
      <div className="w-8 h-8 rounded-lg overflow-hidden border border-slate-200 shadow-sm shrink-0 bg-slate-50 flex items-center justify-center">
        <LogoWithFallback
          src={tool.logo_url}
          alt={tool.name}
...
function AlternativeRow({ tool, rank }: { tool: Tool; rank: number }) {
  ...
  return (
    <button
      ...
      className="group w-full bg-slate-50 border border-slate-200 rounded-xl px-4 sm:px-5 py-3.5 sm:py-4 flex items-center gap-3 sm:gap-4 text-left transition-all duration-200 hover:border-amber-200 hover:shadow-md hover:-translate-y-0.5"
    >
      {/* Rank */}
      <span
      ...
      {/* Logo */}
      <div className="w-10 h-10 sm:w-11 sm:h-11 rounded-xl overflow-hidden border border-slate-200 shadow-sm shrink-0 bg-slate-50 flex items-center justify-center">
        <LogoWithFallback
          src={tool.logo_url}
          alt={tool.name}
          className="w-full h-full object-cover"
...
function PopularStackCard({
  ...
  return (
    <button
      ...
      className="group bg-slate-50 border border-slate-200 rounded-2xl p-4 sm:p-5 text-left transition-all duration-200 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-amber-200 w-full"
    >
      <div className="flex items-center gap-3 mb-3">
        <div className="w-11 h-11 rounded-xl overflow-hidden border border-slate-200 shadow-sm shrink-0 bg-slate-50 flex items-center justify-center">
          <LogoWithFallback
            src={tool.logo_url}
            alt={tool.name}
            className="w-full h-full object-cover"
      ...
      <p className="text-[12px] text-slate-600 line-clamp-2 mb-3 leading-relaxed">
        {tool.tagline}
      </p>
...
function PopularCardSkeleton() {
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 shadow-sm animate-pulse">
      <div className="flex items-center gap-3 mb-3">
...
function AlternativeRowSkeleton() {
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 sm:px-5 py-3.5 sm:py-4 flex items-center gap-3 sm:gap-4 shadow-sm animate-pulse">
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <section className="relative border-b border-slate-200 pt-[60px] lg:pt-[64px] overflow-hidden" style={{ background: '#F8FAFC' }}>
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-[30px] pb-[26px] relative z-[1]">
          {/* Breadcrumb */}
          <Breadcrumbs items={[{ label: 'Alternatives' }]} className="mb-3 sm:mb-4" />
          ...
          <div className="flex flex-col items-center text-center mb-6">
            ...
            <p className="text-[15px] text-slate-600 font-normal mt-2 leading-relaxed max-w-[540px]">
              Search any stack and discover top-rated alternatives.
            </p>

            {/* Search bar — centered under subtitle */}
            <div className="relative w-full max-w-[400px] mt-4">
              ...
              {searchOpen &&
                ...
                searchResults.length > 0 && (
                  <div className="absolute top-full left-0 right-0 z-20 mt-1.5 bg-slate-50 border-[1.5px] border-slate-200 rounded-xl shadow-xl overflow-hidden">
              ...
              {searchOpen &&
                ...
                searchResults.length === 0 &&
                !loading && (
                  <div className="absolute top-full left-0 right-0 z-20 mt-1.5 bg-slate-50 border-[1.5px] border-slate-200 rounded-xl shadow-lg p-4 text-center">
      ...
      <main className="flex-1 min-w-0">
        ...
        {selectedProduct && (
          <section className="bg-slate-50/50 py-8 sm:py-10 border-b border-slate-200">
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
              {/* Selected stack header */}
              <div className="bg-slate-50 border-[1.5px] border-amber-200 rounded-2xl p-5 sm:p-6 mb-6 shadow-sm">
                <div className="flex items-center gap-4 flex-wrap">
                  <div className="w-14 h-14 rounded-xl overflow-hidden border border-slate-200 shadow-sm shrink-0 bg-slate-50 flex items-center justify-center">
                    <LogoWithFallback
        ...
        {!selectedProduct && (
          <section className="bg-slate-50/50 py-10 sm:py-12 border-b border-slate-200">
            <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
              <div className="mb-8">
                ...
                <p className="text-[14px] text-slate-600 max-w-[480px]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
              {categories.map((cat) => (
                <button
                  ...
                  onClick={() =>
                    router.push(
                      `/c/${cat.name.toLowerCase().replace(/&/g, 'and').replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '')}-tools`
                    )
                  }
                  className="group bg-slate-50 border border-slate-200 rounded-xl px-4 sm:px-5 py-3.5 flex items-center gap-3 text-left transition-all duration-200 hover:border-amber-200 hover:shadow-md w-full"
                >
