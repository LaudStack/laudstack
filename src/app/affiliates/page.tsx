  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <PageHero
        ...
        subtitle="Earn passive income recommending LaudStack to your audience."
        accent="amber"
        layout="centered"
        size="md"
      >
        <div className="flex flex-wrap items-center justify-center gap-3">
          ...
          <a href="#tiers" className="inline-flex items-center gap-1.5 px-5 py-2.5 rounded-[10px] bg-slate-50 border-[1.5px] border-slate-200 text-slate-700 font-semibold text-sm no-underline hover:border-slate-300 transition-colors">
      ...
      <div className="max-w-[1400px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {HOW_IT_WORKS.map(({ step, title, desc }) => (
              <div key={step} className="bg-slate-50 border border-slate-200 rounded-2xl p-5">
        ...
        <div id="tiers" className="mb-16 scroll-mt-24">
          ...
          <p className="text-slate-600 text-sm text-center mb-8">All commissions are recurring — you earn every month your referral stays subscribed.</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {TIERS.map((tier) => (
              <div key={tier.name} className={`relative bg-slate-50 border-2 rounded-2xl p-6 ${tier.color}`}>
                {tier.badge && (
                ...
                <div className="text-2xl sm:text-4xl font-black text-amber-400 mb-1">{tier.commission}</div>
        ...
        <div className="mb-16 bg-slate-50 border border-slate-200 rounded-2xl p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {[
              ...
              { icon: Shield, title: 'Trusted Brand', desc: 'LaudStack is a trusted platform for verified SaaS & AI stack reviews. Your audience will recognize the quality.' },
        ...
        <div id="apply" className="scroll-mt-24">
          <div className="max-w-2xl mx-auto">
            ...
            <p className="text-slate-600 text-sm text-center mb-8">Free to join. Approved within 24 hours. No minimum traffic required.</p>
            ...
            {submitted ? (
              <div className="bg-green-500/10 border border-green-500/30 rounded-2xl p-10 text-center">
                ...
                <p className="text-slate-600 text-sm">We'll review your application and send your affiliate link to <strong className="text-slate-900">{form.email}</strong> within 24 hours.</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="bg-slate-50 border border-slate-200 rounded-2xl p-8 space-y-4">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-4 py-2.5 rounded-xl bg-slate-100 border border-slate-300 text-slate-900 text-sm placeholder-slate-400 focus:outline-none focus:border-amber-500"
                    />
                  </div>
                  <div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full px-4 py-2.5 rounded-xl bg-slate-100 border border-slate-300 text-slate-900 text-sm placeholder-slate-400 focus:outline-none focus:border-amber-500 resize-none"
                  />
                </div>
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-2.5 rounded-xl bg-slate-100 border border-slate-300 text-slate-900 text-sm focus:outline-none focus:border-amber-500"
                  >
