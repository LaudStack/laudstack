    ...
    <div className="flex flex-col gap-5">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div className="md:col-span-2">
          ...
          <div className="text-xs text-slate-500 mt-1 text-right">{form.tagline.length}/120</div>
        </div>
        <div>
...
function StepCategory({ form, setForm }: { form: FormData; setForm: (f: FormData) => void }) {
  ...
  return (
    <div className="flex flex-col gap-7">
      ...
      <div>
        <label className={labelBase}>Tags <span className="text-xs font-normal text-slate-500">up to 8</span></label>
            ...
            {form.plans.map((plan, i) => (
              <div key={i} className="p-4 bg-slate-50 border border-slate-200 rounded-xl flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Plan {i + 1}</span>
                  {form.plans.length > 1 && (
...
function StepMedia({ form, setForm }: { form: FormData; setForm: (f: FormData) => void }) {
  ...
  return (
    <div className="flex flex-col gap-7">
      <div>
        <label className={labelBase}>Screenshot URLs <span className="text-xs font-normal text-slate-500">up to 5</span></label>
        <p className="text-xs text-slate-500 mb-3">Add URLs to screenshots hosted on your site, Imgur, or any public CDN.</p>
      ...
      <div>
        ...
        <div className="text-xs text-slate-500 mt-1">YouTube, Loom, or Vimeo links are supported</div>
      </div>
          ...
          <div className="divide-y divide-slate-100">
            {section.items.map(item => (
              <div key={item.key} className="flex items-start gap-4 px-4 py-3">
                <span className="text-xs font-semibold text-slate-500 w-24 flex-shrink-0 mt-0.5">{item.key}</span>
                <span className="text-sm text-slate-700 flex-1">{item.val || <span className="text-slate-500 italic">Not provided</span>}</span>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="text-center py-8">
      ...
      <div className="flex flex-col sm:flex-row gap-3 justify-center">
        ...
        <button onClick={() => router.push("/launches")}
          className="flex items-center gap-2 bg-slate-50 hover:bg-slate-50 text-slate-700 font-bold px-6 py-3 rounded-xl transition-colors text-sm border border-slate-200">
          Browse Launches
        </button>
      </div>
    </div>
        ...
        {submitted ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-10">
            ...
            <div className="lg:sticky lg:top-24 lg:self-start">
              <div className="mb-6">
                <Link href="/launchpad" className="inline-flex items-center gap-1.5 text-sm text-slate-600 hover:text-slate-700 transition-colors mb-4">
                  <ArrowLeft className="w-3.5 h-3.5" />
                  Back to LaunchPad
                </Link>
                <h1 className="text-xl font-black text-slate-900 mb-1">Launch Your Stack</h1>
                <p className="text-sm text-slate-600">Complete all steps to submit your listing</p>
              </div>
            ...
            <div>
              <div className="mb-8 pb-6 border-b border-slate-200">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <div className="text-xs font-bold text-slate-500 uppercase tracking-wider">Step {currentStep} of {STEPS.length}</div>
