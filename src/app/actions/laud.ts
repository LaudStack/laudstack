...
import { getCurrentUser, requireAuth, requireAdmin } from "@/lib/admin-auth";
...
export async function toggleLaud(toolId: number): Promise<{
  ...
  const tool = await db.query.tools.findFirst({
    ...
    columns: { id: true, name: true, slug: true, upvoteCount: true, claimedBy: true },
  });
  ...
  if (existing.length > 0) {
    ...
    // Send "tool lauded" email to founder (fire-and-forget)
    if (tool.claimedBy) {
      import("@/server/email").then(async ({ sendToolLaudedEmail }) => {
        try {
          ...
          if (founder?.email) {
            await sendToolLaudedEmail(
              founder.email,
              tool.name ?? "Your Stack",
              tool.slug ?? "",
              user.name || "Someone",
              updated[0]?.upvoteCount ?? 1
            );
          }
        } catch (e) {
          console.error("[Laud] email notification error:", e);
        }
      }).catch(e => console.error("[Laud] email import error:", e));
    }
