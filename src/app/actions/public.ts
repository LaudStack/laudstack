...
import { eq, desc, and, or, ilike, sql, count, avg, ne, gte, inArray } from "drizzle-orm";
...
import { sendNewReviewEmail, sendAdminNewReviewAlert } from "@/server/email";
...
export async function getActiveDeals() {
  return db
    ...
    .orderBy(
      sql`CASE ${deals.placement} WHEN 'deal_of_day' THEN 0 WHEN 'featured' THEN 1 WHEN 'pinned' THEN 2 ELSE 3 END`,
...
// getCurrentUser imported from @/lib/admin-auth
...
export async function submitReview(data: {
  ...
  // Also always notify admins about every new review
  try {
    const tool = await db.query.tools.findFirst({
    ...
    // Admin notification for every review (especially flagged ones)
    if (tool) {
      sendAdminNewReviewAlert({
        toolName: tool.name,
        toolSlug: tool.slug,
        reviewerName: user.firstName || user.name || "Anonymous",
        reviewerEmail: user.email || "",
        rating: data.rating,
        title: data.title.trim(),
        excerpt: data.body.trim(),
        isSpam,
      }).catch(e => console.error('[submitReview] admin alert error:', e));
    }

    // Founder notification (only for non-spam reviews)
    if (!isSpam && tool?.claimedBy && tool.claimedBy !== user.id) {
      ...
      // Also send email notification to the founder
      ...
      if (founder?.email) {
        await sendNewReviewEmail(
          ...
          tool.name,
          tool.slug,
          user.firstName || 'A LaudStack user',
          data.rating,
          data.body.trim()
        ).catch(e => console.error('[submitReview] email error:', e));
      }
    }
  } catch (e) {
