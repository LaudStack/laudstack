...
import {
  Users, Flame, Filter, Star, ExternalLink,
...
function CardSkeleton() {
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-5 animate-pulse flex items-center gap-5">
      <div className="w-9 h-9 rounded-[10px] bg-slate-200 flex-shrink-0" />
...
function CommunityPickCard({
  ...
  return (
    <div
      className={`bg-white rounded-2xl px-5 py-4 sm:px-6 sm:py-5 flex items-center gap-3 sm:gap-5 cursor-pointer transition-all duration-200 relative overflow-hidden group ${
        isTop3
          ...
          : 'border-[1.5px] border-slate-200 shadow-sm hover:shadow-[0_8px_28px_rgba(15,23,42,0.10)] hover:border-amber-200'
    ...
    >
      ...
      {/* Logo */}
      <div className="w-[52px] h-[52px] rounded-xl flex-shrink-0 border border-slate-200 bg-slate-50 overflow-hidden flex items-center justify-center hidden sm:flex">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1 flex-wrap">
          ...
          {tool.is_featured && (
            <span className="hidden sm:inline-flex items-center gap-1 text-xs font-extrabold uppercase tracking-wide px-1.5 py-0.5 rounded bg-orange-50 text-amber-700 border border-amber-200">
        ...
        <p className="text-[13px] text-slate-600 mb-2 line-clamp-1 leading-relaxed">
          {tool.tagline}
        </p>
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex items-center gap-0.5">
            ...
            <span className="text-[11px] text-slate-500">({tool.review_count})</span>
          </div>
          {/* Category */}
          <span className="text-[11px] text-slate-500 font-semibold bg-slate-50 px-2 py-0.5 rounded-md border border-slate-200 hidden md:inline">
            {tool.category}
          </span>
          {/* Pricing */}
      ...
      <a
        ...
        className="flex-shrink-0 w-9 h-9 rounded-[10px] border-[1.5px] border-slate-200 bg-slate-50 flex items-center justify-center text-slate-500 hover:border-amber-500 hover:text-amber-600 hover:bg-amber-50 transition-all hidden sm:flex"
      >
...
export default function CommunityPicks() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <Navbar />
      <div className="h-[60px] lg:h-[64px] flex-shrink-0" />
      ...
      <section className="relative border-b border-slate-200 py-8 sm:py-12 overflow-hidden" style={{ background: '#F8FAFC' }}>
        ...
        <div aria-hidden className="absolute" style={{ top: '-20%', right: '-5%', width: '500px', height: '400px', background: 'radial-gradient(ellipse at center, rgba(244, 63, 94, 0.05) 0%, transparent 70%)', pointerEvents: 'none' }} />
        <div aria-hidden className="absolute left-0 top-0 bottom-0" style={{ width: '3px', background: '#F43F5E' }} />
          ...
          <div className="flex flex-col sm:flex-row items-start justify-between gap-6">
            ...
            <div className="flex gap-4 flex-wrap">
              {[
                ...
                { icon: TrendingUp, value: totalVotes.toLocaleString(), label: 'Total Votes Cast' },
                { icon: TrendingUp, value: `${allTools.length}+`, label: 'Tools Ranked' },
              ].map(({ icon: Icon, value, label }) => (
                <div key={label} className="text-center bg-slate-50 border border-slate-200 rounded-xl px-4 py-3.5 min-w-[90px]">
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <div className="relative flex-[1_1_180px] max-w-[320px]">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-[15px] h-[15px] text-slate-500" />
              <input
                value={searchQuery}
                onChange={e => setSearchQuery(e.target.value)}
                placeholder="Search community picks..."
                className="w-full pl-9 pr-3 h-[38px] text-[13px] text-slate-900 bg-white border-[1.5px] border-slate-200 rounded-[9px] outline-none transition-colors focus:border-amber-500"
              />
            </div>
                ...
                <button
                  ...
                  className={`px-3.5 py-1.5 rounded-lg text-xs font-bold transition-all ${
                    timePeriod === key
                      ...
                      : 'border-[1.5px] border-slate-200 bg-white text-slate-500 hover:border-slate-300'
                  }`}
                >
                  {label}
                </button>
      ...
      <main className="flex-1 min-w-0 py-10 pb-16">
        <div>
          {/* Results header */}
          ...
          {visibleCount < filteredTools.length && (
            <div className="text-center mt-8">
              <button
                ...
                className="inline-flex items-center gap-2 px-7 py-3 rounded-[10px] border-[1.5px] border-slate-200 text-[13px] font-bold text-slate-700 bg-slate-50 shadow-sm hover:border-amber-500 hover:text-amber-700 hover:bg-amber-50 transition-all"
              >
      ...
      <section className="bg-slate-50 border-t border-slate-200 py-12">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h2 className="text-2xl font-black text-slate-900 mb-2.5 tracking-tight">
