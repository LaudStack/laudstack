  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <PageHero
        ...
        subtitle="Reviews, comparisons, and guides for smarter SaaS decisions."
        accent="amber"
        layout="centered"
        size="md"
      >
        {/* Article category quick-links */}
        <div className="flex flex-col gap-1.5 shrink-0 min-w-0 sm:min-w-[180px]">
          {CATEGORIES.filter(c => c !== 'All').map(cat => {
            ...
            return (
              ...
              >
                ...
                <span className="text-[11px] font-bold text-slate-500 bg-slate-100 px-1.5 py-px rounded-full">{count}</span>
              </button>
            );
          })}
        </div>
      ...
      <div className="max-w-[1400px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-10">

        {/* Featured Article */}
        {featured && activeCategory === 'All' && (
          <Link href={`/blog/${featured.slug}`}>
            <div className="group relative rounded-2xl overflow-hidden border border-slate-200 cursor-pointer mb-8 sm:mb-12 hover:border-amber-500/40 transition-all duration-300">
              ...
              <div className="relative p-5 sm:p-8 md:p-12 max-w-2xl">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <span className="px-2.5 py-1 rounded-full bg-slate-100 text-slate-600 text-xs font-medium">{featured.category}</span>
                </div>
                <h2 className="text-2xl md:text-3xl font-black text-slate-900 mb-3 group-hover:text-amber-600 transition-colors">
                  {featured.title}
                </h2>
                <p className="text-slate-500 text-sm sm:text-base mb-4 sm:mb-6 leading-relaxed">{featured.excerpt}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {rest.map((article) => (
            <Link key={article.id} href={`/blog/${article.slug}`}>
              <article className="group bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden cursor-pointer hover:border-amber-500/40 hover:bg-slate-100/80 transition-all duration-200 h-full flex flex-col">
                ...
                <div className="p-5 flex flex-col flex-1">
                  <div className="flex items-center gap-2 mb-3">
                    <span className="px-2 py-0.5 rounded-full bg-slate-100 border border-slate-300 text-xs font-medium text-slate-500">
                      {article.category}
                    </span>
                  ...
                  <p className="text-sm text-slate-600 line-clamp-2 leading-relaxed mb-4 flex-1">{article.excerpt}</p>
        ...
        {rest.length === 0 && (
          <div className="text-center py-20">
            ...
            <p className="text-slate-600 text-sm">Check back soon — we publish new content weekly.</p>
          </div>
        )}
