...
export default function NotFound() {
  return (
    <div className="min-h-screen bg-white text-slate-900 flex flex-col">
      <Navbar />
      <div className="mt-[60px] lg:mt-[64px] flex-1 flex flex-col items-center justify-center px-4 py-20">
          ...
          <div className="grid grid-cols-2 gap-3">
            {QUICK_LINKS.map(({ label, href, icon: Icon }) => (
              <Link key={label} href={href}>
                <div className="flex items-center gap-2 bg-slate-100/60 hover:bg-slate-200/60 border border-slate-300/60 hover:border-amber-500/30 rounded-xl px-4 py-3 cursor-pointer transition-all group">
        ...
        <div className="mt-10 flex items-center gap-2 text-slate-600 text-sm">
          ...
          <span>Try searching with <kbd className="bg-slate-100 border border-slate-300 px-1.5 py-0.5 rounded text-xs font-mono text-slate-500">⌘K</kbd></span>
        </div>
      </div>
      <Footer />
    </div>
