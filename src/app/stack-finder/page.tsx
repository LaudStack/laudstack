...
import {
  Compass, ArrowRight, ArrowLeft, Star,
...
function QuestionHeader({ title, subtitle }: { title: string; subtitle: string }) {
  return (
    <div className="text-center mb-6 sm:mb-8">
      <h2 className="text-xl sm:text-2xl lg:text-3xl font-black text-slate-900 tracking-tight mb-1.5 sm:mb-2">
        {title}
      </h2>
        ...
        {categories.map((cat) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xs sm:text-[11px] text-slate-500 font-medium mt-0.5 block leading-tight">
                {cat.description}
              </span>
            </button>
          );
        ...
        {BUSINESS_TYPES.map((bt) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] text-slate-500 font-medium">{bt.desc}</span>
            </button>
          );
        })}
      </div>
        ...
        {TEAM_SIZES.map((ts) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] text-slate-500 font-medium">{ts.desc}</span>
            </button>
          );
        })}
      </div>
        ...
        {BUDGET_OPTIONS.map((bo) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] text-slate-500 font-medium">{bo.desc}</span>
            </button>
          );
        })}
      </div>
...
function StepPricingPrefs({
  ...
  return (
    <div>
      ...
      <p className="text-center text-[11px] text-slate-500 font-medium mt-4">
        ...
        {PRIORITIES.map((p) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] text-slate-500 font-medium leading-tight">{p.desc}</span>
            </button>
          );
        })}
      </div>
...
function ResultCard({ result, rank }: { result: StackFinderResult; rank: number }) {
  ...
  return (
    <div
      className="group bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl p-4 sm:p-6 transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5 hover:border-amber-200 cursor-pointer"
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 sm:gap-4 mt-2 sm:mt-2.5 flex-wrap">
            <div className="flex items-center gap-1">
              ...
              {tool.review_count > 0 && (
                <span className="text-xs sm:text-xs text-slate-500 font-medium">({tool.review_count})</span>
              )}
            </div>
            <div
            ...
            <span
              className={`text-xs sm:text-[11px] font-bold px-2 sm:px-2.5 py-0.5 rounded-md border ${
            ...
            <span className="text-xs sm:text-[11px] font-semibold text-slate-500 bg-slate-50 px-2 sm:px-2.5 py-0.5 rounded-md border border-slate-200">
              {tool.category}
            </span>
          </div>
          ...
          {matchReasons.length > 0 && (
            <div className="mt-2.5 sm:mt-3 flex flex-wrap gap-1 sm:gap-1.5">
              {matchReasons.map((reason, i) => (
                <span
                  key={i}
                  className="inline-flex items-center gap-1 text-xs sm:text-[11px] font-semibold text-amber-700 bg-amber-50 border border-amber-200 px-1.5 sm:px-2 py-0.5 rounded-md"
                >
  ...
  return (
    <div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4 mb-5 sm:mb-6">
        ...
        <button
          onClick={onRefine}
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl border-2 border-slate-200 bg-slate-50 text-sm font-bold text-slate-700 hover:border-slate-300 hover:bg-slate-50 transition-all cursor-pointer"
        >
      ...
      {response.results.length > 0 && (
        <div className="flex items-center gap-2 mb-4 sm:mb-5 overflow-x-auto pb-1">
          ...
          <span className="text-xs sm:text-xs font-bold text-slate-500 uppercase tracking-wider flex-shrink-0">Sort:</span>
      ...
      {sortedResults.length > 0 ? (
        ...
        <div className="text-center py-12 sm:py-16 bg-slate-50 rounded-2xl border border-slate-200">
          <Search className="w-10 h-10 sm:w-12 sm:h-12 text-slate-200 mx-auto mb-3 sm:mb-4" />
          <h3 className="text-base sm:text-lg font-black text-slate-700 mb-2">No matching stacks found</h3>
          <p className="text-xs sm:text-sm text-slate-500 mb-5 sm:mb-6 max-w-md mx-auto px-4">
            Try expanding your categories, budget, or filters.
          </p>
          <button
            onClick={onRefine}
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <section className="relative border-b border-slate-200 pt-[60px] lg:pt-[64px] overflow-hidden" style={{ background: '#F8FAFC' }}>
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-[30px] pb-[26px] relative z-[1]">
          {/* Breadcrumb */}
          <Breadcrumbs items={[{ label: 'Stack Finder' }]} className="mb-3 sm:mb-4" />

          <div className="flex flex-col items-center text-center mb-2 sm:mb-3">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-amber-50 border border-amber-200 flex items-center justify-center mb-2">
            ...
            <p className="text-xs sm:text-sm text-slate-500 font-medium mt-1 max-w-[540px]">
