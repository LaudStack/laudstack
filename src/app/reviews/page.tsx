...
function HeroSkeleton() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', minWidth: '160px', flexShrink: 0 }}>
      ...
      <div style={{ marginTop: '6px', paddingTop: '8px', borderTop: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', gap: '6px' }}>
...
function ReviewCard({ review, onAuthRequired, initialVoted = false }: { review: any; onAuthRequired: () => void; initialVoted?: boolean }) {
  ...
  return (
    <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 sm:p-6 hover:shadow-md hover:border-slate-300 transition-all">
      {/* Header row */}
      <div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-4 mb-4">
        <div className="flex items-center gap-3">
        ...
        {review.tool && (
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 hover:border-slate-300 transition-all shrink-0 self-start"
          >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-4">
          {review.pros && (
            <div className="bg-emerald-50 rounded-xl p-3 border border-emerald-100">
              <div className="text-xs font-bold text-emerald-700 uppercase tracking-wider mb-1">Pros</div>
              <p className="text-xs text-emerald-800 leading-relaxed">{review.pros}</p>
            </div>
          )}
          {review.cons && (
            <div className="bg-rose-50 rounded-xl p-3 border border-rose-100">
              <div className="text-xs font-bold text-rose-700 uppercase tracking-wider mb-1">Cons</div>
    ...
    return (
      <div className="min-h-screen bg-white">
        ...
        breadcrumbs={[{ label: 'Reviews' }]}
          ...
          subtitle="Verified reviews only. No fake ratings, no paid placements."
          accent="amber"
          layout="centered"
          size="md"
        >
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-10">
          <div className="bg-slate-50 rounded-2xl border border-slate-200 p-4 mb-6 shadow-sm">
