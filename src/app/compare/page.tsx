...
export default function Compare() {
  ...
  if (toolsLoading || (hasUrlTools && !hydrated)) {
    return (
      <div className="min-h-screen flex flex-col bg-white">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-5 px-4 sm:px-6 py-20 text-center">
          ...
          <p className="text-base text-slate-600 max-w-[400px] leading-relaxed">
  ...
  function getCellValue(tool: typeof tools[0], key: string, featMap: Record<string, boolean>): React.ReactNode {
    switch (key) {
      case 'category':       return <span className="text-[13px] font-semibold text-slate-700">{tool.category}</span>;
      case 'pricing_model': {
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <section className="relative border-b border-slate-200 pt-[60px] lg:pt-[64px] overflow-hidden" style={{ background: '#F8FAFC' }}>
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-[30px] pb-[26px] relative z-[1]">
          {/* Breadcrumb */}
          <Breadcrumbs items={[{ label: 'Comparisons', href: '/comparisons' }, { label: 'Compare' }]} className="mb-3 sm:mb-4" />
          ...
          <div className="flex flex-col items-center text-center mb-2">
              ...
              <p className="text-[15px] text-slate-600 font-normal mt-2 leading-relaxed max-w-[560px]">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-lg border-[1.5px] border-slate-200 bg-slate-50 text-slate-700 font-semibold text-[13px] hover:border-slate-300 transition-colors"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> Back
          </button>
          <div className="flex items-center gap-2 px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg max-w-[400px] overflow-hidden">
      ...
      {/* ── Main content ── */}
      <div className="max-w-[1400px] mx-auto w-full px-4 sm:px-10 pt-10 pb-20">
        ...
        <div className="grid gap-4 mb-7 items-stretch" style={{ gridTemplateColumns: gridCols, minWidth: gridMinWidth }}>
          ...
          {tools.map((tool, i) => (
            <div
              key={tool.id}
              className="bg-slate-50 rounded-2xl border border-slate-200 p-5 shadow-sm flex flex-col gap-3 relative"
            >
              {/* Remove button */}
              <button
                onClick={() => { remove(tool.id); if (selected.length <= 2) router.push('/'); }}
                className="absolute top-3 right-3 w-[26px] h-[26px] rounded-lg border border-slate-200 bg-slate-50 flex items-center justify-center text-slate-500 hover:bg-red-50 hover:border-red-200 hover:text-red-500 transition-all"
              >
                ×
              </button>

              {/* Logo */}
              <div className="w-14 h-14 rounded-[14px] border-[1.5px] border-slate-200 bg-slate-50 overflow-hidden flex items-center justify-center">
              ...
              <div>
                <div className="flex items-center gap-1.5 mb-1">
                  <h2 className="text-[17px] font-black text-slate-900 tracking-tight">{tool.name}</h2>
                  {tool.is_verified && <ShieldCheck className="w-3.5 h-3.5 text-green-500 flex-shrink-0" />}
                </div>
                <p className="text-[13px] text-slate-600 leading-snug">{tool.tagline}</p>
              </div>
              ...
              <a
                ...
                className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-[10px] font-bold text-[13px] transition-all mt-auto ${
                  i === 0
                    ...
                    : 'bg-slate-50 text-slate-700 border-[1.5px] border-slate-200 hover:border-slate-300 hover:bg-slate-100'
                }`}
              >
        ...
        {SECTION_ROWS.map((section) => (
          <div
            key={section.section}
            className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden shadow-sm mb-5"
          >
            {/* Section header */}
            <div className="px-3 sm:px-6 py-3.5 bg-slate-50 border-b border-slate-200">
            ...
            {section.rows.map((row, ri) => (
              <div
                key={row.key}
                className={`grid gap-4 px-3 sm:px-6 py-3.5 items-center ${ri % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} ${ri < section.rows.length - 1 ? 'border-b border-slate-200' : ''}`}
                ...
                {tools.map((tool, ti) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {winner && <span className="ml-1.5 text-xs font-bold text-green-700 bg-green-100 px-1.5 py-px rounded-full">Best</span>}
                    </div>
                  );
                })}
              </div>
        ...
        {/* ── Tags row ── */}
        <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden shadow-sm mb-5">
          <div className="px-3 sm:px-6 py-3.5 bg-slate-50 border-b border-slate-200">
            <span className="text-xs font-extrabold text-slate-900 uppercase tracking-widest">Tags</span>
          </div>
          <div className="grid gap-4 px-3 sm:px-6 py-4 items-start" style={{ gridTemplateColumns: gridCols, minWidth: gridMinWidth }}>
            <div className="text-[13px] font-semibold text-slate-500">Keywords</div>
            {tools.map(tool => (
              <div key={tool.id} className="flex flex-wrap gap-1.5">
                {tool.tags.slice(0, 6).map(tag => (
                  <span key={tag} className="text-[11px] font-semibold px-2 py-0.5 rounded-lg bg-slate-50 text-slate-600 border border-slate-200">#{tag}</span>
                ))}
              </div>
            ))}
          </div>
        ...
        <div className="bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden shadow-sm mb-5">
          ...
          {allTierNames.map((tierName, ri) => (
            <div
              key={tierName}
              className={`grid gap-4 px-3 sm:px-6 py-4 items-start ${ri % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'} ${ri < allTierNames.length - 1 ? 'border-b border-slate-200' : ''}`}
              style={{ gridTemplateColumns: gridCols, minWidth: gridMinWidth }}
            >
              {/* Row label */}
              <div className="text-[13px] font-bold text-slate-700 pt-1">{tierName}</div>
                ...
                return (
                  <div key={tools[ti].id} className="flex flex-col gap-1.5">
                    <div className="flex items-baseline gap-1">
                      ...
                      {tier.period && <span className="text-[11px] text-slate-500 font-medium">{tier.period}</span>}
                      {tier.highlighted && (
                        <span className="text-xs font-bold px-1.5 py-px rounded-full bg-amber-50 text-amber-700 border border-amber-200 ml-1">Popular</span>
                      )}
                    </div>
                    <p className="text-[11px] text-slate-600 leading-snug">{tier.description}</p>
                    {tier.features && tier.features.length > 0 && (
                      <ul className="mt-1 flex flex-col gap-0.5">
                        {tier.features.slice(0, 4).map((f: string, fi: number) => (
                          <li key={fi} className="flex items-start gap-1.5 text-[11px] text-slate-700">
                            <CheckCircle2 className="w-[11px] h-[11px] text-green-500 flex-shrink-0 mt-px" />
                            {f}
                          </li>
                        ))}
                        {tier.features.length > 4 && (
                          <li className="text-[11px] text-slate-500">+{tier.features.length - 4} more features</li>
                        )}
                      </ul>
                    )}
                  </div>
          ...
          <div className="px-6 py-3 bg-slate-50 border-t border-slate-200">
            <p className="text-[11px] text-slate-500">Pricing is indicative. Visit each product's website for the latest plans and pricing.</p>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-2xl px-8 py-7 flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-[13px] text-slate-300">Visit each stack's page to read full reviews and make your choice.</p>
          </div>
          <div className="flex gap-2.5 flex-wrap items-center">
            {/* Share button in CTA row */}
            <button
              onClick={handleShare}
              className="inline-flex items-center gap-1.5 px-4 py-2.5 rounded-[10px] bg-slate-50/[0.08] text-slate-300 font-bold text-[13px] border border-white/10 hover:bg-amber-500/15 hover:border-amber-500/30 hover:text-amber-500 transition-all"
            >
