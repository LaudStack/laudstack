      ...
      >
        ...
        {uploading ? (
          ...
          <div className="flex flex-col items-center gap-2 py-4">
            ...
            <p className="text-xs text-slate-500">JPEG, PNG, WebP, GIF, SVG — Max {type === "screenshot" ? "10MB" : "5MB"}</p>
          </div>
        )}
      </div>
    </div>
  ...
  const loadTool = useCallback(async () => {
    ...
    try {
      ...
      if (!data) {
        ...
        router.push("/ops-console/stacks/listed");
        return;
      }
  ...
  return (
    <div className="max-w-7xl">
      ...
      <div className="flex items-center gap-4 mb-6">
        <button
          onClick={() => router.push("/ops-console/stacks/listed")}
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-slate-500">ID: {tool.id} · Slug: {tool.slug}</p>
          </div>
        </div>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {founderInfo.founderStatus === 'verified' && (
                      <span className="text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full font-semibold">Verified</span>
                    )}
                    {founderInfo.founderStatus === 'pending' && (
                      <span className="text-xs bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-semibold">Pending</span>
                    )}
                  </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
              {screenshots.map((ss) => (
                <div key={ss.id} className="relative group">
                  ...
                  {ss.caption && (
                    <p className="text-xs text-slate-500 mt-1 truncate">{ss.caption}</p>
                  )}
                </div>
              ))}
            </div>
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-xs text-slate-500">{review.userEmail}</span>
                        {review.isVerified && (
                          <span className="text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full font-semibold">Verified</span>
                        )}
                      </div>
              ...
              {[
                ...
                <div key={m.label} className="p-4 bg-slate-50 rounded-lg">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <span className="text-xs font-semibold text-slate-400 uppercase">{m.label}</span>
                  </div>
