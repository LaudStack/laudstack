...
import LaudViewModal from "@/components/admin/LaudViewModal";
...
export default function AdminLaudsPage() {
  ...
  const [selectedLaud, setSelectedLaud] = useState<any | null>(null);
    ...
    <div className="space-y-6 max-w-7xl">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-600 mt-1">Monitor laud activity, detect fraud, and maintain integrity.</p>
        </div>
        <button
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {activity?.lauds?.map((laud: any) => (
                    <tr key={laud.id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50 cursor-pointer" onClick={() => setSelectedLaud(laud)}>
      ...
      {selectedLaud && (
        <LaudViewModal
          laud={selectedLaud}
          onClose={() => setSelectedLaud(null)}
          onRefresh={() => { loadData(); if (activeTab === "activity") loadActivity(activityPage); setSelectedLaud(null); }}
        />
      )}
    </div>
  );
