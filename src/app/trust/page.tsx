  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 flex flex-col">
      ...
      <PageHero
        ...
        subtitle="How we ensure quality and authenticity in every listing and review."
        accent="green"
        layout="centered"
        size="md"
      />
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 sm:py-16">
        ...
        <div className="mb-10 sm:mb-16">
          <div className="flex items-center gap-2 text-slate-600 text-sm font-medium mb-5 sm:mb-8">
            <Info className="w-4 h-4" />
            Four pillars of trust
          </div>
          <div className="space-y-4 sm:space-y-6">
            {TRUST_PILLARS.map((pillar) => (
              <div key={pillar.title} className="bg-slate-50 border border-slate-200 rounded-2xl p-5 sm:p-8 hover:border-slate-300 transition-colors">
                <div className="flex items-start gap-4">
                  <div className="flex-shrink-0 mt-1">{pillar.icon}</div>
                  <div className="flex-1">
                    <h3 className="text-slate-900 font-bold text-lg sm:text-xl mb-2 sm:mb-3">{pillar.title}</h3>
        ...
        <div className="mb-10 sm:mb-16">
          ...
          <h2 className="text-2xl sm:text-3xl font-black text-slate-900 mb-3">How rankings are calculated</h2>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden">
            <div className="hidden sm:grid grid-cols-3 px-4 sm:px-6 py-3 border-b border-slate-200 bg-slate-100/50 text-slate-500 text-xs font-medium uppercase tracking-wide">
              <span>Signal</span>
              <span className="text-center">Weight</span>
              <span className="text-right">Description</span>
            </div>
            {RANKING_FACTORS.map((factor, i) => (
              <div key={factor.factor} className={`flex flex-col sm:grid sm:grid-cols-3 px-4 sm:px-6 py-3 sm:py-4 gap-1 sm:gap-4 sm:items-center ${i < RANKING_FACTORS.length - 1 ? 'border-b border-slate-200/50' : ''}`}>
                ...
                <span className="text-slate-500 text-xs sm:text-sm sm:text-right">{factor.description}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="mb-10 sm:mb-16">
          ...
          <h2 className="text-2xl sm:text-3xl font-black text-slate-900 mb-3">What we don't allow</h2>
          ...
          <div className="bg-slate-50 border border-rose-500/20 rounded-2xl p-4 sm:p-6">
        ...
        {/* Report */}
        <div className="mb-10 sm:mb-16 bg-slate-50 border border-slate-200 rounded-2xl p-5 sm:p-8">
          <div className="flex items-start gap-4">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-6 sm:p-10 text-center">
          ...
          <h3 className="text-slate-900 font-black text-xl sm:text-2xl mb-3">Founder Guidelines</h3>
