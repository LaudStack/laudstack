  ...
  return (
    ...
    >
      ...
      <div className="p-4 pb-3 flex items-start gap-3">
        ...
        {/* Status badge */}
        {isLive ? (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-amber-50 border border-amber-200 text-xs font-bold text-amber-700 uppercase tracking-wide shrink-0">
      ...
      <div className="px-4 pb-3">
        <p className="text-[13px] text-slate-600 leading-relaxed line-clamp-2 m-0">
          {tool.tagline}
        </p>
      </div>
      ...
      {!isLive && (
        <div className="px-4 pb-3">
          <div className="flex items-center gap-1 justify-center">
            ...
            <span className="text-xs text-slate-300 font-semibold pb-3.5">:</span>
      ...
      {/* Footer */}
      <div className="mt-auto px-4 py-2.5 border-t border-slate-200 flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          <CalendarClock className="w-[13px] h-[13px] text-slate-500" />
          <span className="text-xs text-slate-500 font-medium">{formatDate(tool.launchDate)}</span>
        </div>
        <button
          onClick={(e) => {
            e.stopPropagation();
...
function RecentCard({ tool }: { tool: UpcomingTool }) {
  ...
  return (
    <div
      ...
      className={`bg-slate-50 rounded-xl border border-slate-200 p-3.5 flex items-center gap-3 transition-all duration-150 hover:-translate-y-px hover:shadow-md ${
        tool.slug ? "cursor-pointer" : "cursor-default"
      }`}
    >
      {/* Logo */}
      ...
      <div className="flex flex-col items-end gap-0.5 shrink-0">
        ...
        <span className="text-[11px] text-slate-500">{formatDate(tool.launchDate)}</span>
      </div>
    </div>
  );
}
...
function PageSkeleton() {
  return (
    <div className="max-w-[1400px] mx-auto w-full px-3 sm:px-6 lg:px-8 pt-6 pb-12">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="animate-pulse bg-slate-100 rounded-[14px] h-[220px]" />
        ))}
      </div>
    </div>
  );
...
export default function UpcomingLaunches() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <PageHero
        breadcrumbs={[{ label: 'Launches', href: '/launches' }, { label: 'Upcoming Launches' }]}
        eyebrow="Upcoming"
        title="Upcoming Launches"
        subtitle="Upcoming stacks — be the first to discover them."
        accent="amber"
        layout="centered"
        size="md"
      />
      ...
      {/* Filter bar */}
      <div className="bg-white/95 backdrop-blur-sm border-b border-slate-200 sticky top-16 z-20 shadow-sm">
        <div className="max-w-[1400px] mx-auto w-full px-3 sm:px-6 lg:px-8 flex items-center justify-between h-12 gap-3">
          <div className="flex items-center gap-2.5 overflow-auto flex-1">
            <Filter className="w-3.5 h-3.5 text-slate-500 shrink-0" />
            <select
              value={category}
              onChange={(e) => setCategory(e.target.value)}
              className="px-2.5 py-1 rounded-lg border-[1.5px] border-slate-200 text-xs font-semibold text-slate-700 bg-slate-50 cursor-pointer outline-none focus:border-amber-400 transition-colors"
            >
              {allCategories.map((c) => (
      ...
      {/* Content */}
      <div className="max-w-[1400px] mx-auto w-full px-3 sm:px-6 lg:px-8 pt-5 sm:pt-7 pb-8 sm:pb-14 flex-1">
        {loading ? (
          <PageSkeleton />
        ) : (
          <>
            ...
            {hasUpcoming && (
              <section className="mb-8 sm:mb-12">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h2 className="text-lg font-black text-slate-900">Launching Soon</h2>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            {!hasUpcoming && !hasRecent && (
              <div className="text-center py-20">
                ...
                <h3 className="text-xl font-black text-slate-900 mb-2">No launches scheduled yet</h3>
                <p className="text-[15px] text-slate-600 leading-relaxed max-w-[400px] mx-auto mb-6">
            ...
            {/* Bottom CTA */}
            {(hasUpcoming || hasRecent) && (
              <div className="mt-8 sm:mt-12 p-5 sm:p-8 rounded-2xl border border-slate-200 bg-slate-50 text-center">
                <h3 className="text-lg sm:text-xl font-black text-slate-900 mb-2">
                  Want your stack featured here?
                </h3>
                <p className="text-sm sm:text-[15px] text-slate-600 leading-relaxed max-w-[480px] mx-auto mb-4 sm:mb-5">
