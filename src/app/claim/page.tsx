    ...
    <div className="min-h-screen bg-white text-slate-900 flex flex-col">
      ...
      <div className="flex-1">
        ...
        breadcrumbs={[{ label: 'Claim Your Stack' }]}
          ...
          subtitle="Verify ownership and unlock Pro founder features."
          accent="amber"
          layout="centered"
          size="md"
        />
        ...
        <div className="max-w-[1400px] mx-auto px-4 py-12">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            ...
            <div className="lg:col-span-2">
                  ...
                  return (
                    <div key={s} className="flex items-center gap-2">
                      <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition-all ${
                        done ? 'bg-green-400 text-slate-900' : active ? 'bg-amber-400 text-slate-900' : 'bg-slate-100 text-slate-500'
                      }`}>
                      ...
                      {i < 2 && <div className={`h-px w-8 ${done ? 'bg-green-400' : 'bg-slate-200'}`} />}
                    </div>
                  );
                })}
              </div>
              ...
              {/* Step 1: Search */}
              {step === 'search' && (
                <div className="bg-slate-50/60 border border-slate-300/60 rounded-2xl p-6">
                  <h2 className="text-xl font-black text-slate-900 mb-2">Find your stack</h2>
                  <p className="text-slate-600 text-sm mb-6">Search for your stack on LaudStack.</p>
                  <div className="relative">
                    <input
                      ...
                      className="w-full bg-slate-100 border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 text-sm focus:outline-none focus:border-amber-400 transition-colors"
                    />
                    {filteredTools.length > 0 && (
                      <div className="absolute top-full left-0 right-0 mt-2 bg-slate-100 border border-slate-300 rounded-xl overflow-hidden z-10 shadow-xl">
                        {filteredTools.map(tool => (
                          ...
                          >
                            <div className="w-8 h-8 bg-slate-200 rounded-lg flex items-center justify-center text-sm font-black text-amber-400 flex-shrink-0">
                              {tool.name[0]}
                            </div>
                            <div>
                  ...
                  {query.length > 1 && filteredTools.length === 0 && (
                    <div className="mt-4 p-4 bg-slate-100/60 border border-slate-300/40 rounded-xl text-center">
                      <p className="text-slate-600 text-sm mb-3">Product not found on LaudStack?</p>
                      <Link href="/launchpad">
              ...
              {step === 'verify' && selectedTool && (
                <div className="bg-slate-50/60 border border-slate-300/60 rounded-2xl p-6">
                  <div className="flex items-center gap-3 mb-6 p-3 bg-slate-100/60 rounded-xl border border-slate-300/40">
                  ...
                  <p className="text-slate-600 text-sm mb-6">Choose how you'd like to prove you own or represent this product.</p>
                  <div className="space-y-3 mb-6">
                    {([
                      ...
                      <label key={opt.id} className={`flex items-start gap-4 p-4 rounded-xl border cursor-pointer transition-all ${
                        verifyMethod === opt.id ? 'border-amber-400/60 bg-amber-400/5' : 'border-slate-300/60 hover:border-slate-400'
                      }`}>
              ...
              {step === 'details' && (
                <form onSubmit={handleSubmit} className="bg-slate-50/60 border border-slate-300/60 rounded-2xl p-6">
                  <h2 className="text-xl font-black text-slate-900 mb-2">Founder profile</h2>
                  <p className="text-slate-600 text-sm mb-6">Tell us about yourself. This information will be reviewed by our team.</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                    <div>
                      <label className="text-slate-500 text-xs font-semibold mb-1.5 block">Full Name *</label>
                      <input value={founderName} onChange={e => setFounderName(e.target.value)} required placeholder="Your full name" className="w-full bg-slate-100 border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 text-sm focus:outline-none focus:border-amber-400 transition-colors" />
                    </div>
                    <div>
                      <label className="text-slate-500 text-xs font-semibold mb-1.5 block">Work Email *</label>
                      <input value={founderEmail} onChange={e => setFounderEmail(e.target.value)} required type="email" placeholder="alex@yourtool.com" className="w-full bg-slate-100 border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 text-sm focus:outline-none focus:border-amber-400 transition-colors" />
                    </div>
                    <div>
                      <label className="text-slate-500 text-xs font-semibold mb-1.5 block">Role / Title *</label>
                      <input value={founderRole} onChange={e => setFounderRole(e.target.value)} required placeholder="Co-founder & CEO" className="w-full bg-slate-100 border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 text-sm focus:outline-none focus:border-amber-400 transition-colors" />
                    </div>
                    <div>
                      <label className="text-slate-500 text-xs font-semibold mb-1.5 block">Tool Website</label>
                      <input value={website} onChange={e => setWebsite(e.target.value)} placeholder="https://yourtool.com" className="w-full bg-slate-100 border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 text-sm focus:outline-none focus:border-amber-400 transition-colors" />
                    </div>
                  </div>
                  <div className="mb-4">
                    <label className="text-slate-500 text-xs font-semibold mb-1.5 block">LinkedIn Profile</label>
                    <input value={linkedin} onChange={e => setLinkedin(e.target.value)} placeholder="https://linkedin.com/in/yourprofile" className="w-full bg-slate-100 border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 text-sm focus:outline-none focus:border-amber-400 transition-colors" />
                  </div>
                  <div className="mb-6">
                    <label className="text-slate-500 text-xs font-semibold mb-1.5 block">Brief Bio (optional)</label>
                    <textarea value={founderBio} onChange={e => setFounderBio(e.target.value)} rows={3} placeholder="Tell us about your background and why you built this product..." className="w-full bg-slate-100 border border-slate-300 rounded-xl px-4 py-3 text-slate-900 placeholder-slate-400 text-sm focus:outline-none focus:border-amber-400 transition-colors resize-none" />
                  </div>
              ...
              {/* Step 4: Received */}
              {step === 'submitted' && (
                <div className="bg-slate-50/60 border border-green-500/30 rounded-2xl p-8 text-center">
                  ...
                  <div className="flex items-center gap-2 bg-slate-100/60 border border-slate-300/40 rounded-xl px-4 py-3 text-sm text-slate-500 mb-6 justify-center">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 justify-center">
                    ...
                    <Link href="/pricing">
                      <button className="bg-slate-100 hover:bg-slate-200 text-slate-900 font-bold px-6 py-3 rounded-xl text-sm transition-colors border border-slate-300">
                        View Pro Features
                      </button>
                    </Link>
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-50/60 border border-slate-300/60 rounded-2xl p-5">
                ...
                <ol className="space-y-2">
                  {['Find your stack on LaudStack', 'Choose a verification method', 'Complete your founder profile', 'Our team reviews in 1–2 days', 'Get your Verified badge'].map((s, i) => (
                    <li key={i} className="flex items-start gap-2 text-xs text-slate-500">
                      <span className="w-4 h-4 bg-slate-100 rounded-full flex items-center justify-center text-slate-500 font-bold flex-shrink-0 mt-0.5 text-xs">{i + 1}</span>
                      {s}
                    </li>
                  ))}
                </ol>
