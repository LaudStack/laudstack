...
export async function POST(request: Request) {
  // ── Auth guard ────────────────────────────────────────────────────────────
  // Method 1: CRON_SECRET header (for cron jobs)
  const authHeader =
  ...
  if (!isAuthorized) {
    try {
      ...
      if (user) {
        ...
        if (dbUser && isRoleAtLeast(dbUser.role, "manager")) {
          isAuthorized = true;
        }
      }
    } catch {
      ...
      const [lastWeekUpvotes] = await db
        ...
        .where(
          and(
            ...
            sql`${upvotes.createdAt} < ${sevenDaysAgo.toISOString()}`
          )
        );
      ...
      const [lastWeekReviews] = await db
        ...
        .where(
          and(
            ...
            sql`${reviews.createdAt} < ${sevenDaysAgo.toISOString()}`,
