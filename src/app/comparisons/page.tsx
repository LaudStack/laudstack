...
function MatchupCard({ toolA, toolB }: { toolA: Tool; toolB: Tool }) {
  ...
  return (
    <button
      ...
      className="group bg-slate-50 border border-slate-200 rounded-2xl p-5 text-left cursor-pointer transition-all duration-200 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-amber-200 w-full"
    >
      {/* Two logos with VS badge */}
      <div className="flex items-center justify-center gap-4 mb-4">
        <div className="text-center">
          <div className="w-12 h-12 rounded-xl overflow-hidden border border-slate-200 shadow-sm mx-auto bg-slate-50 flex items-center justify-center">
            <LogoWithFallback
              src={toolA.logo_url}
              alt={toolA.name}
...
function StackPicker({
  ...
  if (selected) {
    return (
      <div className="flex-1 min-w-0 sm:min-w-[240px] bg-slate-50 border-[1.5px] border-slate-200 rounded-xl px-4 py-3 flex items-center gap-3">
        <div className="w-10 h-10 rounded-lg overflow-hidden border border-slate-200 shadow-sm shrink-0 bg-slate-50 flex items-center justify-center">
          <LogoWithFallback
        ...
        <button
          onClick={onClear}
          className="text-slate-500 hover:text-slate-600 p-1 rounded-md hover:bg-slate-100 transition-colors"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
  ...
  return (
    <div className="flex-1 min-w-0 sm:min-w-[240px] relative">
      ...
      <div className="flex items-center bg-white rounded-xl border-[1.5px] border-slate-200 overflow-hidden focus-within:border-amber-300 focus-within:ring-1 focus-within:ring-amber-200 transition-all">
        <Search className="ml-3.5 w-[14px] h-[14px] text-slate-500 shrink-0" />
        <input
          value={query}
          onChange={(e) => {
          ...
          className="flex-1 px-3 py-3 text-[14px] text-slate-900 bg-transparent border-none outline-none placeholder:text-slate-500"
        />
      </div>
      {open && query.trim() && results.length > 0 && (
        <div className="absolute top-full left-0 right-0 z-20 mt-1 bg-slate-50 border-[1.5px] border-slate-200 rounded-xl shadow-xl overflow-hidden">
          {results.map((tool) => (
            <button
              key={tool.slug}
              onMouseDown={() => {
            ...
            >
              <div className="w-7 h-7 rounded-md overflow-hidden border border-slate-200 bg-slate-50 flex items-center justify-center shrink-0">
                <LogoWithFallback
                  src={tool.logo_url}
                  alt={tool.name}
                  className="w-full h-full object-cover"
...
function CategoryRowSkeleton() {
  return (
    <div className="bg-slate-50 border border-slate-200 rounded-xl px-4 sm:px-5 py-3.5 sm:py-4 flex items-center gap-3 sm:gap-4 animate-pulse">
    ...
    <div className="min-h-screen flex flex-col bg-white">
      ...
      <section className="relative border-b border-slate-200 pt-[60px] lg:pt-[64px] overflow-hidden" style={{ background: '#F8FAFC' }}>
        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-[30px] pb-[26px] relative z-[1]">
          {/* Breadcrumb */}
          <Breadcrumbs items={[{ label: 'Comparisons' }]} className="mb-3 sm:mb-4" />
          ...
          <div className="flex flex-col items-center text-center mb-6">
            ...
            <p className="text-[15px] text-slate-600 font-normal mt-2 leading-relaxed max-w-[540px]">
              Compare AI and SaaS stacks side by side to find the best fit.
            </p>
          </div>
        </div>
      </section>
              ...
              <div className="flex flex-col gap-3">
                {categoryMatchups.map((cat) => (
                  <button
                    ...
                    className="group bg-slate-50 border border-slate-200 rounded-xl px-4 sm:px-5 py-3.5 sm:py-4 flex items-center gap-3 sm:gap-4 text-left transition-all duration-200 hover:border-amber-200 hover:shadow-md w-full"
                  >
                    ...
                    <div className="hidden sm:flex items-center gap-2 shrink-0">
                      <div className="w-8 h-8 rounded-lg overflow-hidden border border-slate-200 shadow-sm bg-slate-50 flex items-center justify-center">
                        <LogoWithFallback
