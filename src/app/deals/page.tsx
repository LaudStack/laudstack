 ...
 * Design: white bg (#F8FAFC), amber accents, rose urgency, no gradients
...
import { claimDeal, getUserClaimedDealIds } from '@/app/actions/user';
import { createFounderDeal } from '@/app/actions/founder';
...
function mapDbDealToUiDeal(d: DbDeal, index: number): Deal {
  ...
  // Determine deal type from the DB field, falling back to inference
  let type: Exclude<DealType, 'all'> = 'discount';
  if (d.dealType === 'lifetime') type = 'lifetime';
  else if (d.dealType === 'free_trial') type = 'free-trial';
  else if (d.dealType === 'exclusive') type = 'exclusive';
  else type = 'discount';
  ...
  // Determine urgency (overrides other badges)
  ...
  // Check placement for deal of the day / featured / pinned
  const isDealOfDay = d.placement === 'deal_of_day' || (index === 0 && !d.placement);
  ...
  const colorPalette = [
    { bg: '#0C1830', text: '#ffffff' },
    { bg: '#5178FF', text: '#ffffff' },
    { bg: '#5178FF', text: '#ffffff' },
    { bg: '#5178FF', text: '#ffffff' },
    { bg: '#5178FF', text: '#ffffff' },
    { bg: '#5178FF', text: '#ffffff' },
    { bg: '#FBBF24', text: '#0C1830' },
  ...
  return {
    ...
    dealPrice: d.dealPrice ?? (pct > 0 ? `${pct}% OFF` : 'Special Offer'),
    discount: pct > 0 ? `${pct}% OFF` : (d.dealPrice ?? 'Deal'),
    code: d.couponCode ?? '',
    expiresAt,
    ...
    featured: isDealOfDay || d.placement === 'featured',
...
function CountdownPill({ expiresAt, large = false }: { expiresAt: string; large?: boolean }) {
  ...
  const seg = (val: number, lbl: string) => (
    <div className="flex flex-col items-center min-w-[40px]">
      ...
      <span className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">{lbl}</span>
    </div>
  );
...
function DealOfDaySpotlight({ deal, claimedDealIds, onClaim }: { deal: Deal; claimedDealIds: Set<string>; onClaim: (dealId: string) => Promise<void> }) {
  const { isAuthenticated } = useAuth();
  const [copied, setCopied] = useState(false);
  const [claiming, setClaiming] = useState(false);
  const [showAuthModal, setShowAuthModal] = useState(false);
  const alreadyClaimed = claimedDealIds.has(deal.id);
  ...
  const handleClaim = async () => {
    ...
    if (alreadyClaimed) {
      // Already claimed — just open the deal URL
  ...
  return (
    <section className="mb-10">
      ...
      {/* Spotlight card */}
      <div className="bg-slate-50 border-2 border-amber-200 rounded-2xl overflow-hidden shadow-[0_4px_24px_rgba(245,158,11,0.10)]">

        {/* Top banner */}
        <div className="bg-amber-50 border-b border-amber-100 px-4 sm:px-6 py-3 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start gap-5 mb-5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-slate-600 text-sm mb-2">{deal.tagline}</p>
                  <div className="flex items-center gap-3 flex-wrap">
                    ...
                    {deal.rating > 0 && (
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-slate-500">({deal.reviews.toLocaleString()} reviews)</span>
                      </div>
                    )}
                  </div>
                </div>
            ...
            <div className="lg:w-72 shrink-0">
              <div className="bg-slate-50 border border-slate-200 rounded-2xl p-6">
                ...
                <div className="mb-5">
                  {deal.originalPrice && (
                    <div className="text-xs font-semibold text-slate-500 line-through mb-1">{deal.originalPrice}</div>
                  )}
                  <div className="text-2xl sm:text-4xl font-black text-slate-900 tracking-tight mb-1">{deal.dealPrice}</div>
                ...
                >
                  {claiming ? (
                    ...
                    <><CheckCircle className="w-4 h-4" /> Claimed — Visit Deal</>
                  ) : (
                    <><ArrowUpRight className="w-4 h-4" /> Claim This Deal</>
                  )}
                </button>
                <p className="text-center text-xs text-slate-500 mt-2.5">{alreadyClaimed ? 'You\'ve already claimed this deal' : 'No credit card required to claim'}</p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <AuthGateModal open={showAuthModal} onClose={() => setShowAuthModal(false)} action="claim_deal" />
    </section>
  );
}

// ── Deal Card ──────────────────────────────────────────────────────────────────
function DealCard({ deal, featured, claimedDealIds, onClaim }: { deal: Deal; featured?: boolean; claimedDealIds: Set<string>; onClaim: (dealId: string) => Promise<void> }) {
  const { isAuthenticated } = useAuth();
  const [copied, setCopied] = useState(false);
  const [claiming, setClaiming] = useState(false);
  const [showAuthModal, setShowAuthModal] = useState(false);
  ...
  const borderCls = isUrgent ? 'border-red-200' : featured ? 'border-amber-200' : 'border-slate-200 hover:border-slate-300';
  const shadowCls = featured ? 'shadow-[0_4px_16px_rgba(245,158,11,0.10)]' : 'shadow-sm';

  return (
    <div
      className={`bg-slate-50 rounded-2xl border-[1.5px] ${borderCls} ${shadowCls} hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 overflow-hidden flex flex-col h-full`}
    >
      ...
      {deal.placement === 'pinned' && !isUrgent && !featured && (
        <div className="px-4 py-1.5 bg-blue-50 border-b border-blue-200 flex items-center gap-1.5">
          <TrendingUp className="w-3 h-3 text-blue-600" />
          <span className="text-[11px] font-bold text-blue-800">Pinned · Promoted Deal</span>
        </div>
      )}
      {featured && !isUrgent && (
      ...
      <div className="p-5 flex-1 flex flex-col">
        ...
        <div className="flex items-start gap-3.5 mb-3.5">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
              <h3 className="text-[15px] font-black text-slate-900 tracking-tight">{deal.name}</h3>
              {deal.badge && (
            ...
            <div className="flex items-center gap-1.5">
              <span className="text-xs font-semibold px-2 py-0.5 rounded-md bg-slate-50 text-slate-500 border border-slate-200">
                {deal.category}
              </span>
              {deal.rating > 0 && (
                <div className="flex items-center gap-0.5">
                  <Star className="w-2.5 h-2.5 fill-amber-400 text-amber-400" />
                  <span className="text-[11px] font-bold text-slate-700">{deal.rating.toFixed(1)}</span>
                  <span className="text-xs text-slate-500">({deal.reviews.toLocaleString()})</span>
                </div>
              )}
            </div>
          </div>
        ...
        {/* ── Pricing row ── */}
        <div className="flex items-center justify-between px-3.5 py-2.5 bg-slate-50 rounded-xl border border-slate-200 mb-3.5">
          <div>
            {deal.originalPrice && (
              <div className="text-xs text-slate-500 line-through mb-px">{deal.originalPrice}</div>
            )}
        ...
        {deal.maxClaims > 0 && (
          <div className="mb-4">
            <div className="flex justify-between mb-1">
              ...
              <span className={`text-xs font-bold ${claimedPct >= 90 ? 'text-red-400' : 'text-slate-400'}`}>{claimedPct}%</span>
            </div>
        ...
        <div className="flex gap-2 mt-auto">
          ...
          <button
            onClick={handleClaim}
            disabled={claiming}
            className={`py-2 px-4 rounded-lg font-extrabold text-xs flex items-center gap-1 transition-all shrink-0 ${
              alreadyClaimed
                ? 'bg-emerald-500 text-white hover:bg-emerald-400'
                : 'bg-amber-500 text-slate-900 hover:bg-amber-400 shadow-[0_2px_8px_rgba(245,158,11,0.25)]'
            } disabled:opacity-60`}
          >
            {claiming ? (
...
function LaunchDealModal({ onClose }: { onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    ...
    dealType: 'discount' as 'discount' | 'lifetime' | 'free_trial' | 'exclusive',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      // createFounderDeal requires a toolId — the founder must own the tool.
      // We look up the founder's tools by name to find a match.
      const { getFounderTools } = await import('@/app/actions/founder');
      const result = await getFounderTools();
      if (!result.success || !result.tools || result.tools.length === 0) {
        toast.error('You need to be a verified founder with a listed product to create deals. Visit LaunchPad to get started.');
        onClose();
        window.location.href = '/launchpad';
        return;
      }
      // Try to match by tool name
      const matchedTool = result.tools.find(
        (t: { name: string }) => t.name.toLowerCase() === form.toolName.trim().toLowerCase()
      );
      if (!matchedTool) {
        toast.error(`No matching product found for "${form.toolName}". Please use the exact name of your listed product, or create deals from your Founder Dashboard.`);
        return;
      }
      const dealResult = await createFounderDeal({
        toolId: matchedTool.id,
        title: `${form.discount}% off ${form.toolName}`,
        description: form.description || `Get ${form.discount}% off ${form.toolName}`,
        dealType: form.dealType,
        discountPercent: parseInt(form.discount) || undefined,
      ...
      if (dealResult.success) {
        ...
        toast.error(dealResult.error || 'Failed to submit deal. Please try again.');
      }
    } catch (err) {
      console.error('[LaunchDealModal] Error:', err);
      toast.error('Failed to submit deal. Please try again or use the Founder Dashboard.');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const selectCls = 'w-full px-3.5 py-2.5 rounded-lg border-[1.5px] border-slate-200 text-sm text-slate-900 outline-none bg-slate-50 focus:border-amber-400 focus:ring-1 focus:ring-amber-200 transition-colors';
  ...
  return (
    ...
    >
      <div className="bg-white rounded-2xl w-full max-w-[520px] mx-4 sm:mx-auto shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="px-4 pt-4 pb-3.5 border-b border-slate-200 flex items-center justify-between sticky top-0 bg-white z-10">
          <div>
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-4 flex flex-col gap-4">
            {step === 1 ? (
              <>
                <p className="text-sm text-slate-600">Tell us about the product and the deal you want to share.</p>
                {[
              ...
              <>
                <p className="text-sm text-slate-600">Now tell us the deal details and how to reach you.</p>
          ...
          {/* Footer */}
          <div className="px-4 sm:px-7 py-4 border-t border-slate-200 flex justify-between gap-3">
            {step === 2 && (
              <button type="button" onClick={() => setStep(1)}
                className="px-5 py-2.5 rounded-lg border-[1.5px] border-slate-200 bg-slate-50 text-sm font-bold text-slate-700 cursor-pointer hover:bg-slate-50 transition-colors">
                Back
              </button>
            )}
            {step === 1 ? (
              ...
              <button type="submit" disabled={submitting}
                className="ml-auto px-6 py-2.5 rounded-lg bg-amber-500 text-sm font-extrabold text-slate-900 cursor-pointer flex items-center gap-1.5 hover:bg-amber-400 transition-colors disabled:opacity-60">
                {submitting ? <><Loader2 className="w-3.5 h-3.5 animate-spin" /> Submitting...</> : <>Launch Deal <CheckCircle className="w-3.5 h-3.5" /></>}
              </button>
            )}
          </div>
        </form>
...
export default function Deals() {
  ...
  const handleClaimDeal = useCallback(async (dealId: string) => {
    const result = await claimDeal(parseInt(dealId));
    if (result.success) {
      setClaimedDealIds(prev => new Set([...prev, dealId]));
      // Update local deal claim count
      setAllDeals(prev => prev.map(d => d.id === dealId ? { ...d, claimed: d.claimed + 1 } : d));
      if (result.couponCode) {
        await navigator.clipboard.writeText(result.couponCode);
        toast.success(`Deal claimed! Code "${result.couponCode}" copied to clipboard.`);
      } else {
        toast.success('Deal claimed successfully!');
      }
      // Open deal URL if available
      ...
      setClaimedDealIds(prev => new Set([...prev, dealId]));
      toast.info('You\'ve already claimed this deal.');
    } else {
      toast.error(result.error || 'Failed to claim deal');
    }
  }, [allDeals]);

  const parseParams = () => {
    if (typeof window === 'undefined') return { type: 'all' as DealType, category: 'All Categories', sort: 'featured' };
  ...
  const featuredDeals = allDeals.filter(d => d.placement === 'featured' && !d.dealOfDay);
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <PageHero
        breadcrumbs={[{ label: 'SaaS Deals' }]}
        eyebrow="Exclusive SaaS Deals"
        title="Save on the products your team uses"
        subtitle="Exclusive discounts and lifetime deals for the LaudStack community."
        accent="amber"
        layout="centered"
        size="md"
      />

      <div className="max-w-[1400px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-10">

        {/* ── Loading state ── */}
        {loading && (
          <div className="flex flex-col items-center justify-center py-20">
            <Loader2 className="w-8 h-8 text-amber-500 animate-spin mb-4" />
            <p className="text-slate-600 text-sm font-medium">Loading deals...</p>
          </div>
        )}
          ...
          <>
            ...
            {allDeals.length === 0 && (
              <div className="text-center py-20 bg-slate-50 border border-slate-200 rounded-2xl mb-12">
                ...
                <p className="text-slate-600 text-sm mb-4">Check back soon — new deals are added every week.</p>
                <button
            ...
            {allDeals.length > 0 && (
              <>
                ...
                {!hasFilters && dealOfDay && <DealOfDaySpotlight deal={dealOfDay} claimedDealIds={claimedDealIds} onClaim={handleClaimDeal} />}

                {/* ── Featured Deals spotlight (only when no filters active) ── */}
                {!hasFilters && featuredDeals.length > 0 && (
                  <section className="mb-10">
                    <div className="flex items-center gap-3 mb-5">
                      ...
                      <h2 className="text-lg font-black text-slate-900 tracking-tight">Featured Deals</h2>
                      <span className="bg-amber-100 text-amber-800 text-xs font-bold px-2.5 py-1 rounded-full border border-amber-200">
                        Hand-picked by LaudStack
                      </span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                      {featuredDeals.map(deal => <DealCard key={deal.id} deal={deal} featured claimedDealIds={claimedDealIds} onClaim={handleClaimDeal} />)}
                    </div>
                  </section>
                )}
                ...
                {!hasFilters && lifetimeDeals.length > 0 && (
                  <section className="mb-10">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                      {lifetimeDeals.map(deal => <DealCard key={deal.id} deal={deal} claimedDealIds={claimedDealIds} onClaim={handleClaimDeal} />)}
                    </div>
                  </section>
                )}
                  ...
                  <div className="flex flex-wrap gap-3 items-center">
                        ...
                        <button
                          ...
                          className={`inline-flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-bold transition-all ${
                            activeType === id
                              ...
                              : `bg-slate-100 hover:bg-slate-200 ${color || 'text-slate-600'}`
                          }`}
                        >
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <select value={activeCategory} onChange={e => setActiveCategory(e.target.value)}
                        className="text-sm font-medium text-slate-700 bg-slate-100 border-0 rounded-xl px-3 py-2 focus:outline-none focus:ring-2 focus:ring-amber-400 cursor-pointer">
                    ...
                    {hasFilters && (
                      <button onClick={clearFilters}
                        className="inline-flex items-center gap-1.5 px-3 py-2 rounded-xl text-sm font-bold text-slate-500 hover:text-slate-900 hover:bg-slate-100 transition-all ml-auto">
                ...
                {filtered.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mb-12">
                    {filtered.map(deal => (
                      <DealCard key={deal.id} deal={deal} featured={!!deal.featured && !hasFilters} claimedDealIds={claimedDealIds} onClaim={handleClaimDeal} />
                    ))}
                  </div>
                ) : (
                  <div className="text-center py-20 bg-slate-50 border border-slate-200 rounded-2xl mb-12">
                    ...
                    <p className="text-slate-600 text-sm mb-4">Try a different deal type or category.</p>
            ...
            {/* ── Featured Stacks ── */}
            <div className="mb-10">
              <FeaturedStacksSidebar limit={4} title="Featured" variant="inline" />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-10">
              {[
                ...
                <div key={title} className={`bg-slate-50 border border-slate-200 rounded-2xl p-5 flex items-start gap-4`}>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-2xl p-8 flex flex-col md:flex-row items-center justify-between gap-6">
              <div>
                ...
                <p className="text-slate-600 text-sm">Launch a deal for your stack and reach potential customers on LaudStack.</p>
              </div>
              <button
                onClick={() => setShowSubmit(true)}
