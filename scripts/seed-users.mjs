...
const users = [
  { name: "LaudStack Team", email: "team@laudstack.com", role: "admin" },
  { name: "Alex Johnson", email: "alex@example.com", role: "user" },
  { name: "Sarah Chen", email: "sarah@example.com", role: "user" },
  { name: "Mike Williams", email: "mike@example.com", role: "user" },
  { name: "Emily Davis", email: "emily@example.com", role: "user" },
];

for (const u of users) {
  const fakeId = crypto.randomUUID();
  await sql`INSERT INTO users (supabase_id, email, name, role, created_at, updated_at) VALUES (${fakeId}, ${u.email}, ${u.name}, ${u.role}, NOW(), NOW()) ON CONFLICT DO NOTHING`;
  console.log("Created:", u.name);
}

// Now seed reviews
const allUsers = await sql`SELECT id, name FROM users`;
allUsers.forEach(u => console.log("User id:", u.id, u.name));

const toolRows = await sql`SELECT id, slug FROM tools LIMIT 15`;
const existingReviews = await sql`SELECT count(*) FROM reviews`;
...
for (const tool of toolRows) {
  for (let i = 0; i < 3; i++) {
    ...
    await sql`
      INSERT INTO reviews (tool_id, user_id, rating, title, body, pros, cons, is_verified, helpful_count, created_at, updated_at)
      VALUES (${tool.id}, ${userId}, ${review.rating}, ${review.title}, ${review.body}, ${review.pros}, ${review.cons}, true, ${Math.floor(Math.random() * 50)}, NOW() - ${daysAgo + ' days'}::interval, NOW())
    `;
  }
