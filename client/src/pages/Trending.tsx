      ...
      <div className="bg-white border-b border-gray-200 pt-20">
        <div className="max-w-[1300px] mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="grid grid-cols-3 gap-4 mt-8 max-w-lg">
            ...
            <div className="bg-gray-100/60 border border-gray-300 rounded-xl p-4 text-center">
              <div className="text-2xl font-black text-slate-900">+{avgChange}</div>
              <div className="text-xs text-slate-500 mt-0.5 font-medium">Avg Gain</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-[1300px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-8">
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="w-10 shrink-0 text-center">
                  <span className={`text-2xl font-black ${i === 0 ? 'text-amber-400' : i === 1 ? 'text-slate-600' : i === 2 ? 'text-amber-700' : 'text-slate-500'}`}>
                    {i + 1}
                  </span>
                </div>

                {/* Logo */}
                <div className="w-12 h-12 shrink-0 rounded-xl bg-gray-100 border border-gray-300 flex items-center justify-center overflow-hidden">
                  {tool.logo_url ? (
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <h3 className="text-base font-bold text-slate-900 group-hover:text-amber-400 transition-colors">{tool.name}</h3>
                  ...
                  <p className="text-sm text-slate-500 mt-0.5 truncate">{tool.tagline}</p>
                  <div className="flex items-center gap-4 mt-2">
                    <span className="flex items-center gap-1 text-xs text-slate-500">
                      <Star className="h-3 w-3 text-amber-400 fill-amber-400" />
                      <span className="font-semibold text-slate-900">{tool.average_rating.toFixed(1)}</span>
                    </span>
                    <span className="flex items-center gap-1 text-xs text-slate-500">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full bg-gray-100 text-slate-600 font-medium">{tool.pricing_model}</span>
                  </div>
                </div>
        ...
        {trendingTools.length === 0 && (
          <div className="text-center py-24">
            ...
            <h3 className="text-xl font-bold text-slate-900 mb-2">No trending tools in this category</h3>
            <p className="text-slate-500 text-sm">Try selecting "All Categories" to see all trending tools</p>
          </div>
        )}
      </div>
