 ...
 * Sorted by upvote count; filterable by category and time period.
...
export default function CommunityPicks() {
  ...
  const handleVote = (id: string) => {
    if (votedIds.has(id)) {
      toast.info('You already voted for this tool.');
      return;
    }
  ...
  const filteredTools = useMemo(() => {
    ...
    // Sort by upvote_count (community votes)
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 24, flexWrap: 'wrap' }}>
            <div>
              ...
              <p style={{ fontSize: 16, color: '#64748B', maxWidth: 520, lineHeight: 1.65, margin: 0 }}>
                These are the tools that the LaudStack community loves most — ranked purely by community upvotes. No editorial bias, just real user enthusiasm.
              </p>
            </div>
            {/* Stats row */}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <div className="flex gap-1.5 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
                {['All', ...CATEGORIES.filter(c => c.name !== 'All').map(c => c.name)].slice(0, 8).map(cat => (
                  <button
                    key={cat}
      ...
      <section style={{ background: '#FFFFFF', borderTop: '1px solid #E2E8F0', padding: '48px 0' }}>
        <div className="max-w-[1300px] mx-auto px-6 lg:px-10" style={{ textAlign: 'center' }}>
          ...
          <h2 style={{ fontFamily: "'Inter', sans-serif", fontSize: 26, fontWeight: 800, color: '#171717', margin: '0 0 10px', letterSpacing: '-0.02em' }}>
            Have a tool the community should know about?
          </h2>
          <p style={{ fontSize: 15, color: '#64748B', margin: '0 0 24px', lineHeight: 1.65 }}>
            Submit it via LaunchPad and let the community vote it up.
          </p>
          <a
            href="/launchpad"
            style={{
          ...
          >
            Submit via LaunchPad <ArrowRight style={{ width: 14, height: 14 }} />
          </a>
        </div>
      </section>
