...
// Featured categories shown in a larger 2-col layout
...
const stagger = {
  ...
  visible: (i: number) => ({
    opacity: 1,
    y: 0,
    transition: { delay: i * 0.035, duration: 0.32, ease: [0.25, 0.46, 0.45, 0.94] as [number, number, number, number] },
  }),
};
...
function CategoryCard({
  ...
  cat: { name: string; icon: string; count: number; description: string };
  index: number;
  large?: boolean;
}) {
...
export default function Categories() {
  ...
  const allCats = CATEGORIES.filter(c => c.name !== 'All');
  ...
  const avgRating = (MOCK_TOOLS.reduce((s, t) => s + t.average_rating, 0) / MOCK_TOOLS.length).toFixed(1);
    ...
    <div className="min-h-screen flex flex-col" style={{ background: '#FFFFFF' }}>
        ...
        >
          {[
            ...
            <div key={s.label} style={{ textAlign: 'center', minWidth: 80 }}>
              <div style={{ fontSize: 20, fontWeight: 800, color: '#0F172A', letterSpacing: '-0.02em', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 3 }}>
                {s.value}
      ...
      {featured.length > 0 && (
        <section className="max-w-[1300px] mx-auto px-6 lg:px-10" style={{ paddingTop: 32, paddingBottom: 8 }}>
          ...
          >
            {featured.map((cat, i) => (
              <CategoryCard key={cat.name} cat={cat} index={i} large />
            ))}
          </div>
        </section>
      )}
      ...
      {rest.length > 0 && (
        <section className="max-w-[1300px] mx-auto px-6 lg:px-10" style={{ paddingTop: 32, paddingBottom: 48 }}>
          ...
          >
            {rest.map((cat, i) => (
              <CategoryCard key={cat.name} cat={cat} index={i + featured.length} />
            ))}
          </div>
        </section>
      )}
